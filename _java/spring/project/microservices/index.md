...
flowchart LR
    ...
    B2(数据库设计约定)===B2-1(库名与应用名尽量一致)
    B2===B2-2(表名字段名必须使用小写字母或数字 禁止出现数字开头)
    B2===B2-3(表名不使用复数名词)
    B2===B2-4(表的命名最好是加上 业务名称_表的作用  如 edu_teacher)
    B2===B2-5(表必备三字段 id gmt_create gmt_modified)
    end
...
- 创建`college_service`服务父模块，继承自[父工程创建](#父工程创建)
